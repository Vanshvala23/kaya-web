  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/70 flex items-center justify-center p-4">
      <div className="bg-white rounded-3xl max-w-md w-full p-6 relative">
        ...
        <h3 className="text-2xl font-bold text-center mb-6">Bridal Makeup Brochure</h3>
        <form className="space-y-4">
          <input placeholder="Full Name" className="w-full border p-3 rounded-xl" />
          <input placeholder="Email" className="w-full border p-3 rounded-xl" />
          <input placeholder="Phone" className="w-full border p-3 rounded-xl" />
          <button className="w-full bg-[#7b1e3a] text-white py-3 rounded-xl font-bold">Download</button>
        </form>
      </div>
    </div>
  );
  ...
  return (
    <>
      ...
      <section className="relative h-[90vh] flex items-center justify-center text-white">
        <img
          src={bridal1}
          className="absolute inset-0 w-full h-full object-cover object-[30%_10%] filter brightness-145"
        />
        <div className="absolute inset-0 bg-gradient-to-r from-[#7b1e3a]/90 to-transparent"></div>
        <div className="relative z-10 max-w-6xl px-6">
          <span className="bg-pink-600 text-xs px-4 py-1 rounded-full">BRIDAL SPECIAL</span>
          <h1 className="text-5xl md:text-7xl font-serif font-bold mt-6">Professional <span className="text-pink-200">Bridal Makeup</span></h1>
          <p className="mt-6 max-w-xl">Become a certified bridal makeup artist and master luxury bridal looks.</p>
          <div className="mt-8 flex gap-4">
            <button onClick={() => setModal(true)} className="bg-white text-[#7b1e3a] px-8 py-3 rounded-full font-bold flex items-center gap-2"><Download size={18}/> Brochure</button>
            <button className="border border-white px-8 py-3 rounded-full">Book Demo</button>
          </div>
        </div>
      </section>
      ...
      <section className="py-20 bg-white">
        <div className="max-w-6xl mx-auto px-6 grid md:grid-cols-2 gap-16">
          <div>
            <h2 className="text-3xl font-serif font-bold mb-6">Course Curriculum</h2>
            {modules.map((m,i)=>(
              <div key={i} className="border rounded-2xl mb-4">
                <button onClick={()=>setActive(active===i?null:i)} className="w-full flex justify-between p-5 font-bold">
                  {m.title}
                  <ChevronDown className={active===i?"rotate-180": ""} />
                </button>
                {active===i && <p className="p-5 text-sm text-gray-600">{m.content}</p>}
              </div>
            ))}
          </div>
          <div>
            <div className="bg-gray-900 text-white p-8 rounded-3xl mb-6">
              <h3 className="font-bold mb-4 flex items-center gap-2"><Package/> Bridal Kit</h3>
              {kit.map((k,i)=>(<p key={i} className="text-sm">• {k.name} – {k.desc}</p>))}
            </div>
            <div className="bg-gray-100 p-6 rounded-3xl">
              <h4 className="font-bold">Course Fee</h4>
              <p className="text-3xl font-extrabold text-[#7b1e3a]">₹ {fees.total.toLocaleString()}</p>
            </div>
          </div>
        </div>
      </section>
          ...
          <div className="grid md:grid-cols-2 gap-6">
            {batches.map(b=>(
              <div key={b.id} className="bg-white p-6 rounded-2xl shadow">
                ...
                <span className="text-xs text-green-600 font-bold">{b.status}</span>
              </div>
            ))}
          </div>
        </div>
      ...
      {/* MOBILE CTA */}
      <div className="fixed bottom-0 left-0 w-full bg-white p-4 flex gap-4 md:hidden">
        <a href="tel:+918872500500" className="flex-1 bg-gray-100 py-3 rounded-xl text-center font-bold"><Phone/> Call</a>
        <button onClick={()=>setModal(true)} className="flex-1 bg-[#7b1e3a] text-white py-3 rounded-xl font-bold flex items-center justify-center gap-2"><ArrowRight/> Apply</button>
      </div>
    </>
  );
}
